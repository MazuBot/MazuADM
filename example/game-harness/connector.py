        ...
        try:
            ...
            while not stop_event.is_set():
                ...
                if message.get('type') != 'flag_created':
                    continue
                item = flag_item_from_payload(message.get('data', {}))
                if item:
                    flag_queue.put(item)
                    logger.info('flag enqueued from websocket: %s', item.flag_id)
        except Exception:
            logger.exception('websocket error; reconnecting in %ss', WS_RECONNECT_DELAY_SECS)
...
def round_monitor_loop(stop_event: threading.Event, game_server: GameServer, api: MazuAPI):
    ...
    while not stop_event.is_set():
        try:
            game_round = game_server.fetch_round()
        except Exception:
            logger.exception('failed to fetch game round')
            time.sleep(ROUND_POLL_INTERVAL_SECS)
            continue
        if current_round != game_round:
            current_round = game_round
            logger.info('game round changed to %s', current_round)
            # TODO: implement round sync logic with MazuADM.
        time.sleep(ROUND_POLL_INTERVAL_SECS)
...
def run_connector():
    ...
    threads = [
        threading.Thread(
            target=ws_flag_listener_loop,
            args=(stop_event, flag_queue, api),
